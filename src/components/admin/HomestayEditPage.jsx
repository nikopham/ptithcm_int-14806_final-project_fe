...
export default function HomestayEditPage() {
  ...
  const [initialAmenityIds, setInitialAmenityIds] = useState([]);
  ...
  useEffect(() => {
    async function loadDetail() {
      try {
        const detail = await getHomestayDetailById(dispatch, homestayId);
        ...
        const newForm = {
          ...
          max_children: detail.max_children ?? "",
          room_count: detail.room_count ?? "",
          provinceId: detail.provinceId || "",
          communeId: detail.commune_id || "",
          address: detail.address || "",
        };

        const newImages = (detail.images || []).map((x) => ({
          _id: x._id,
          url: x.url,
          is_thumb: !!x.is_thumb,
        }));

        const newAmenityIds = (detail.amenity || []).map((a) => a._id);
        ...
        setInitialAmenityIds(newAmenityIds);

        if (detail.provinceId)
          fetchCommunesByProvinceId(dispatch, detail.provinceId);
      } catch {}
    }
    if (homestayId) loadDetail();
  }, [dispatch, homestayId]);
  ...
  const handleRestore = () => {
    if (initialForm) {
      ...
      if (initialForm.provinceId)
        fetchCommunesByProvinceId(dispatch, initialForm.provinceId);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updated = await updateHomestay(dispatch, payload);
      if (updated?.success) {
        toast.success("Cập nhật homestay thành công");
        navigate(-1);
      }
    } catch (e) {
      toast.error("Không thể cập nhật homestay")
      dispatch(
        setError(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">Sửa Homestay</h2>
