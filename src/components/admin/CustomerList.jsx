...
import { clearCustomers, setError } from "@/features/customer/customerSlice";
...
import { deleteAccount } from "@/services/employeeServices";
...
export default function CustomerList() {
  ...
  const currentQuery = useMemo(() => {
    ...
    // Chuẩn hoá kiểu dữ liệu và default
    const page = Number(qObj.page) || 1;
    const limit = Number(qObj.limit) || 10;

    // Nếu đang filter "all" thì bỏ hẳn param status (để backend hiểu là không filter)
    const status =
  ...
  const handleDelete = async (customer) => {
    try {
      await deleteAccount(dispatch, { id: customer?.account?._id });
