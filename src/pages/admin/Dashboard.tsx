...
import { useGetOverviewQuery, useGetViewsChartQuery, useGetTopGenresQuery } from "@/features/dashboard/dashboardApi";
...
export default function Dashboard() {
  ...
  const { data: viewsChart = [], isLoading: chartLoading } = useGetViewsChartQuery(selectedYear);
  const { data: topGenres = [], isLoading: genresLoading } = useGetTopGenresQuery();
    ...
    <div className="space-y-8 px-4 py-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
        {overviewLoading ? (
          ...
          kpis.map((kpi) => (
            <Card key={kpi.label} className="bg-white border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200">
              <CardContent className="flex items-center gap-4 p-6">
                <div className="grid h-12 w-12 place-items-center rounded-lg" style={{ backgroundColor: "#C40E61" }}>
                ...
                {topGenres.map((genre, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="flex items-center justify-center w-8 h-8 rounded-full font-bold text-white" style={{ backgroundColor: "#C40E61" }}>
                        {index + 1}
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900">{genre.name}</p>
                        <p className="text-sm text-gray-500">{formatNumber(genre.movieCount)} phim</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-bold" style={{ color: "#C40E61" }}>
                        {genre.movieCount}
                      </p>
                    </div>
                  </div>
